...
class ClientThread(threading.Thread):
    ...
    def __init__(self, client_socket, addr, mode, topic):
    ...
    def handle_publisher(self):
        ...
        while True:
            ...
            for client in clients:
                if client.mode == "SUBSCRIBER" and client.topic == self.topic:
...
def run_server(port):
    ...
    while isActive.is_set():
        ...
        if server_socket in rlist:
            ...
            mode, topic = data.split('\n')
            client_thread = ClientThread(client_socket, addr, mode, topic)
            client_thread.mode = mode
            client_thread.topic = topic
